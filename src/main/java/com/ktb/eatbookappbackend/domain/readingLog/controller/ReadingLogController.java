...
public class ReadingLogController {
     ...
     * @return ResponseEntity로 마지막으로 읽은 에피소드 정보를 포함하거나, 결과가 없음을 나타내는 성공 메시지를 반환합니다.
     *         응답 본문은 다음 구조를 갖는 SuccessResponseDTO입니다:
     *         - success: 성공 여부를 나타내는 boolean.
     *         - message: 성공 메시지를 나타내는 string.
     *         - data: 다음 key-value 쌍을 포함하는 map:
     *           - continueReadingInfo: Optional로 ContinueReadingInfoDTO를 나타내는 마지막으로 읽은 에피소드 정보.
     *             마지막으로 읽은 에피소드가 없는 경우, 이 필드는 null이 됩니다.
     */
    @Authenticated
